...
Regla de responsabilidad única: `signals.py` = única fuente de verdad para señales. `reporter.py` = único lugar que imprime. `notifier.py` = único lugar que envía mensajes externos. `bot_commands.py` = único lugar que procesa comandos Telegram.

**Notificaciones:** Telegram vía `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` (GitHub Secrets).
**Bot de comandos:** `bot_listener.yml` corre cada 5 min y procesa `/add`, `/remove`, `/list`, `/help` actualizando `watchlist.json` vía GitHub API. Offset de mensajes procesados en `data/last_update_id.txt`.
